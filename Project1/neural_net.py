...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
	# *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
	#softmax 함수의 출력값 계산(Probability)
    exp_i = np.exp(scores)
    sum_i = np.sum(exp_i, axis = 1).reshape(N,1)
    prob = exp_i / sum_i
    prob_y = np.array([])

    for i in range(prob.shape[0]):
      prob_y = np.append(prob_y, prob[i, y[i]])
    Log_prob_y = np.log(prob_y)

    #loss 계산
    loss_ = np.sum(-Log_prob_y) / N
    # L2 regularizaion
    ...
    # dL/dW2=dL/dz*p1^t
    dW2 = h1.T.dot(prob)
    # L2 = reg*(np.sum(W1**2)+np.sum(W2**2))=> dW2 : 2*reg*W2
    ...
    # local gradients dz2/dp1 -> dL/dp1=(dz2/dp1)*dL/dz2=W2^T*dL/dz2

    dlocal = prob.dot(W2.T)

    # dL/dW1=dL/dz1*x^T , dL/dz1->(dp1/dz1)*(dL/dp1)
    dlocal[L2 <= 0] = 0
